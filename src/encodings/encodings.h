...
// Returns the decoded character(s) for a byte, or empty string if it's a control character
inline std::string decodeByteEN_G1(unsigned char byte) {
    static const std::unordered_map<unsigned char, std::string> table = {
        // Space
        {0x7F, " "},
...
inline std::string decodeByte(unsigned char byte, TextEncoding encoding) {
    switch (encoding) {
        case TextEncoding::EN_G1:
            return decodeByteEN_G1(byte);
        case TextEncoding::EN_G2:
            return decodeByteEN_G2(byte);
        case TextEncoding::EN_G3:
            return decodeByteEN_G3(byte);
        case TextEncoding::JP_G1:
            return decodeByteJP_G1(byte);
        case TextEncoding::JP_G2:
            return decodeByteJP_G2(byte);
        case TextEncoding::JP_G3:
            return decodeByteJP_G3(byte);
